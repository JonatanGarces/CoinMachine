...
DM_INTERLACED =2
        ...
        //DEVMODE.dmFields
        private const int DM_FORMNAME = 0x10000;//改變紙張名稱時需在dmFields設置此常數

        private const int DM_PAPERSIZE = 0x0002;//改變紙張類型時需在dmFields設置此常數
        private const int DM_PAPERLENGTH = 0x0004;//改變紙張長度時需在dmFields設置此常數
        private const int DM_PAPERWIDTH = 0x0008;//改變紙張寬度時需在dmFields設置此常數
        private const int DM_DUPLEX = 0x1000;//改變紙張是否雙面列印時需在dmFields設置此常數
        private const int DM_ORIENTATION = 0x0001;//改變紙張方向時需在dmFields設置此常數
        ...
        public static bool ModifyPrinterSettings_V2(string printerName, ref PrinterSettingsInfo PS)
        {
            ...
            PRINTER_INFO_2 pinfo = new PRINTER_INFO_2();
            pinfo = (PRINTER_INFO_2)Marshal.PtrToStructure(ptrPrinterInfo, typeof(PRINTER_INFO_2));
#pragma warning disable CS0219 // La variable 'Temp' está asignada pero su valor nunca se usa
            IntPtr Temp = new IntPtr();
#pragma warning restore CS0219 // La variable 'Temp' está asignada pero su valor nunca se usa
            if (pinfo.pDevMode == IntPtr.Zero)
            {
                // If GetPrinter didn't fill in the DEVMODE, try to get it by calling
                // DocumentProperties...
